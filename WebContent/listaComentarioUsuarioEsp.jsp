...
<table>
				...
				<tr>
				<td><a href="AlteraDadosComentario.jsp?idComentario=<%=comentario.getIdComentario() %> &nomeHospital=<%=comentario.getNomeEstabelecimento()%> &nomecategoria=<%=comentario.getNomecategoria()%> &textoComentario=<%=comentario.getTextoComentario()%> &idUsuario=<%=comentario.getUsuario().getIdUsuario()%> &idHospital=<%=comentario.getHospital().getIdEstabelecimento()%> &idcategoria=<%=comentario.getCategoria().getIdcategoria()%>"><%=comentario.getIdComentario() %></a></td>
